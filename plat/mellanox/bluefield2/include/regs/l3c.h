...
typedef union {
	...
	uint64_t word;
} L3C_COMMON_DEVICE_SECURITY_LEVEL_t;
...
typedef union {
	struct {
		...
		/*
		 * 1 - fully locking ways enabled. check lock range for all
		 * transactions and bypass emi.
		 */
		uint32_t full_waylock_en : 1;
		/*
		 * 1 - partial locking ways enabled. use max access LRU
		 * position configuration.
		 */
		uint32_t ptl_waylock_en  : 1;
		/* Reserved. */
		uint32_t __reserved_3    : 6;
	};
...
/*
 * WAYLOCK_LOW_ADDR.
 * full locking ways feature, lowest address of region
 */
...
typedef union {
	struct {
		/*
		 * lowest Physical Address (PA) of locking ways feature,
		 * included.
		 * 0.5M granularity (e.g. PA[39:19])
		 */
		uint32_t val        : 21;
		/* Reserved. */
	...
	uint32_t word;
} L3C_WAYLOCK_LOW_ADDR_t;


/*
 * WAYLOCK_HIGH_ADDR.
 * full locking ways feature, highest address of region
 */
...
typedef union {
	struct {
		/*
		 * highest Physical Address (PA) of locking ways feature,
		 * included.
		 * 0.5M granularity (e.g. PA[39:19])
		 */
		uint32_t val        : 21;
	...
	uint32_t word;
} L3C_WAYLOCK_HIGH_ADDR_t;


/*
 * WAYLOCK_CFG.
 * partial locking ways feature configurations
 */
...
typedef union {
	struct {
		/*
		 * highest accessible LRU position way in partial way locking
		 */
		uint32_t max_lru_pos : 4;
		/* Reserved. */
		uint32_t __reserved  : 28;
	};
...
typedef union {
	struct {
		 ...
		 * pmr select in bits [6:4], for relevant counters
		 * [7] - must be 0.
		 */
		uint32_t strm_sel   : 8;
		/*
