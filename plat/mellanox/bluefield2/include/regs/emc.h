...
typedef union {
	...
	uint64_t word;
} EMC_COMMON_DEVICE_SECURITY_LEVEL_t;
...
typedef union {
	struct {
		 ...
		 * cycles)
		 */
		uint32_t keep_phase_duration : 5;
	};
...
typedef union {
	struct {
		...
		/*
		 * Write command rank switching penalty
		 * Parameter defined in DDR clocks.
		 * ceil[package rank switch penalty + BL/2]
		 * (number of ddr quiet cycles plus 4)
		 */
		uint32_t prank_switch_gap_wr : 5;
		/* Reserved. */
...
typedef union {
	struct {
		/*
		 * Indicates to which ranks ODT will is given in case of
		 * write command to rank 0 (bitmap)
		 */
		uint32_t bm_pr_0    : 4;
		/*
		 * Indicates to which ranks ODT will is given in case of
		 * write command to rank 1 (bitmap)
		 */
		uint32_t bm_pr_1    : 4;
		/*
		 * Indicates to which ranks ODT will is given in case of
		 * write command to rank 2 (bitmap)
		 */
		uint32_t bm_pr_2    : 4;
		/*
		 * Indicates to which ranks ODT will is given in case of
		 * write command to rank 3 (bitmap)
		 */
		uint32_t bm_pr_3    : 4;
		/*
...
typedef union {
	struct {
		/*
		 * Indicates to which ranks ODT will is given in case of read
		 * command to rank 0 (bitmap)
		 */
		uint32_t bm_pr_0    : 4;
		/*
		 * Indicates to which ranks ODT will is given in case of read
		 * command to rank 1 (bitmap)
		 */
		uint32_t bm_pr_1    : 4;
		/*
		 * Indicates to which ranks ODT will is given in case of read
		 * command to rank 2 (bitmap)
		 */
		uint32_t bm_pr_2    : 4;
		/*
		 * Indicates to which ranks ODT will is given in case of read
		 * command to rank 3 (bitmap)
		 */
		uint32_t bm_pr_3    : 4;
		/* Number of active ODT cycles (DDR clock) */
		uint32_t duration   : 4;
		/*
		 * Number of cycles (DDR clock) that ODT is delayed after CAS
		 * RD command
		 * CL-CWL
		 */
		uint32_t delay      : 5;
...
typedef union {
	struct {
		...
		/*
		 * Select access method to phy DCCM/ICCM:
		 * 0 - through the phy APB interface (write/read).
		 * 1 - direct access from host (write only) through
		 * DDR_PHY_DIR_ACCESS MC register
		 */
		uint32_t sram_access_sel  : 1;
		/* Reserved. */
		uint32_t __reserved_1     : 19;
	};
...
typedef union {
	struct {
		/* Number of PHY_UPD_REQ received by the MC */
		uint32_t req : 16;
		/* Number of PHY_UPD_ACK given by the MC */
		uint32_t ack : 16;
	};
...
typedef union {
	struct {
		 ...
		 * Bits 9:8 - Byte Lane 4
		 * Bits 11:10 - Byte Lane 5
		 * Bits 13:12 - Byte Lane 6
		 * Bits 15:14 - Byte Lane 7
		 * Bits 17:16 - Byte Lane 8
		 */
		uint32_t bl_wld_cfg : 18;
		/* Reserved. */
		uint32_t __reserved : 14;
	};

	uint32_t word;
} EMC_WR_LVL_BL_DELAY_t;
...
typedef union {
	struct {
		 ...
		 * Bits 17:16 - Byte Lane 8
		 */
		uint32_t bl_rld_cfg : 18;
		/* Reserved. */
		uint32_t __reserved : 14;
	};

	uint32_t word;
} EMC_RD_LVL_BL_DELAY_t;
...
typedef union {
	struct {
		/* number of errors per rank */
		uint32_t val : 32;
	};

	uint32_t word;
} EMC_IFC_BIST_RANK_ERR_CNTR_t;
...
typedef union {
	struct {
		 ...
		 * dfi_ctrlupd_req signal must be asserted
		 */
		uint32_t t_ctrlupd_min  : 8;
		/*
		 * Specifies the maximun number of DFI clock cycles that the
		 * dfi_ctrlupd_req signal must be asserted
		 */
		uint32_t t_ctrlupd_max  : 8;
		/* Number of sampling stages of DFI Interface in PHY */
		uint32_t phy_smp_stages : 4;
		/* Number of sampling stages of DFI Interface in MC */
		uint32_t mc_smp_stages  : 4;
		/* Reserved. */
		uint32_t __reserved     : 8;
	};

	uint32_t word;
} EMC_CALIB_CTRLUPD_INTR_CFG_t;
...
typedef union {
	struct {
		/*
		 * Specifies the number of DFI clock cycles that the
		 * dfi_phyupd_req signal must be held high after
		 * dfi_phyupd_ack was asserted
		 */
		uint32_t t_phyupd_type0 : 8;
		/*
		 * Specifies the maximum number of DFI clock cycles in which
		 * dfi_ctrlupd_ack signal must be asserted
		 */
		uint32_t t_phyupd_resp  : 14;
		/* Reserved. */
		uint32_t __reserved     : 10;
	};

	uint32_t word;
} EMC_CALIB_PHYUPD_INTR_CFG_t;
...
typedef union {
	struct {
		/*
		 * dfi_ctrlupd_ack is not asserted before Tctrlupd_min +
		 * 2*SMP stages
		 */
		uint32_t ctrlupd_no_ack_viol        : 1;
		/* Reserved. */
		uint32_t __reserved_0               : 3;
		/*
		 * dfi_ctrlupd_ack is asserted after Tctrlupd_min + 2*SMP
		 * stages
		 */
		uint32_t ctrlupd_ack_min_viol       : 1;
		/* Reserved. */
		uint32_t __reserved_1               : 3;
		/*
		 * dfi_ctrlupd_ack is not deserted before Tctrlupd_max +
		 * 2*SMP stages
		 */
		uint32_t ctrlupd_ack_max_viol       : 1;
		/* Reserved. */
		uint32_t __reserved_2               : 3;
		/*
		 * dfi_phyupd_req is served (dfi_phyupd_ack is asserted) and
		 * not deserted before Tphyupd_type0 + 2*SMP stages after
		 * dfi_phyupd_ack assertion.
		 */
		uint32_t phyupd_serv_req_max_viol   : 1;
		/* Reserved. */
		uint32_t __reserved_3               : 3;
		/*
		 * dfi_phyupd_req is not served (dfi_phyupd_ack is not
		 * asserted) and deserted before Tphyupd_resp.
		 */
		uint32_t phyupd_unserv_req_min_viol : 1;
		/* Reserved. */
		uint32_t __reserved_4               : 3;
		/*
		 * dfi_phyupd_req is not served (dfi_phyupd_ack is not
		 * asserted) and not deserted at Tphyupd_resp.
		 */
		uint32_t phyupd_unserv_req_max_viol : 1;
		/* Reserved. */
		uint32_t __reserved_5               : 3;
		/*
		 * dfi_phyupd_ack is not given before Tphyupd_resp - 2*SMP
		 * stages
		 */
		uint32_t phyupd_unserv_ack_max_viol : 1;
		/* Reserved. */
		uint32_t __reserved_6               : 3;
		/*
		 * functional command is given while dfi_ctrlupd_req is active
		 */
		uint32_t ctrlupd_req_and_cmd_viol   : 1;
		/*
		 * functional command is given while dfi_ctrlupd_ack is active
		 */
		uint32_t ctrlupd_ack_and_cmd_viol   : 1;
		/* Reserved. */
		uint32_t __reserved_7               : 2;
	};

	uint32_t word;
} EMC_CALIB_INTR_CAUSE_t;
...
typedef union {
	struct {
		/*
		 * Serves Host to load DDR PHY SRAMs by direct access (write
		 * only).
		 * Relevant only if sram_access_sel field in PHY_CTRL MC
		 * register is set to 1.
		 * DIRECT ACCESS DATA DECODE:
		 * [30] 0: write to ICCM; 1: write to DCCM
		 * [29:16] SRAM word address
		 * [15:0] data word to write
		 */
		uint32_t data_val : 32;
	};

	uint32_t word;
} EMC_DDR_PHY_DIR_ACCESS_t;
