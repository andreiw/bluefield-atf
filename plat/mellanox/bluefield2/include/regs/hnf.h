...
typedef union {
	...
	uint64_t word;
} HNF_COMMON_DEVICE_SECURITY_LEVEL_t;
...
/*
 * Generic performance module configuration register in network events.
 * Set up the configuration of generic performance module in network events
 */
...
typedef union {
	...
	uint64_t word;
} HNF_GEN_NET_PERFMON_CONFIG_1_t;
...
typedef union {
	...
	uint64_t word;
} HNF_GEN_NET_PERFMON_CONFIG_2_t;
...
typedef union {
	...
	uint64_t word;
} HNF_GEN_NET_PERFMON_CONFIG_3_t;


/*
 * Generic performance value register in network.
 * The register is used to provide interface where software can read the
 * value of generic performance counter in network
 */
