...
typedef union {
	...
	uint64_t word;
} TRIO_COMMON_DEVICE_SECURITY_LEVEL_t;
...
typedef union {
	struct {
		 ...
		 * not attempt to resend config ops. The default one is to
		 * support around 1 second retry latency(expected average
		 * response time is 0.2us)
		 */
		uint64_t limit      : 32;
		/* Reserved. */
		uint64_t __reserved : 32;
